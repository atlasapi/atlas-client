...
public class CachingJaxbUriplayClientTest {
	...
	public void testBasicCachingOfIdentifierQueries() throws Exception {
		...
		context.checking(new Expectations() {{
			one(atlas).contentQuery("atlas/content.xml?uri=1,2"); will(returnValue(result(item1, playlist2)));
		}});
		...
		context.checking(new Expectations() {{
			one(atlas).contentQuery("atlas/content.xml?uri=3"); will(returnValue(result(item3)));
		}});
	...
	public void testNegativeCaching() throws Exception {
		...
		context.checking(new Expectations() {{
			one(atlas).contentQuery("atlas/content.xml?uri=missing,exists"); will(returnValue(result(exists)));
		}});
